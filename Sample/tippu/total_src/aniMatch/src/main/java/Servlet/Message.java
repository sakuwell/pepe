...
public class Message extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// 動物の主キーを指定したメッセージリストを取得して、リクエストスコープに保存
		MessageLogic getMessageLogic = new MessageLogic();
		List<MessageInfo> messageInfoList = getMessageLogic.execute(messageInfoOnSession.getAnimal_key());
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(text != null && text.length() != 0) {
			...
			// つぶやきをつぶやきリストに追加
			MessageInfo messageInfo = new MessageInfo(accountInfoOnSession.getUserName(), text, message_time, messageInfoOnSession.getAnimal_key());
